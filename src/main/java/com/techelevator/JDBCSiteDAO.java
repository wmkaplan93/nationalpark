...
public class JDBCSiteDAO implements SiteDAO {
	...
	public List<Site> mostPopularSites(long campground_id, int fromDate, int toDate) {
		...
		String sqlList = "WITH    reservation AS ( "
				...
				+ "       GROUP BY r.site_id " 
				+ "       ) " 
				+ "SELECT s.site_number, COALESCE(r.rescount, 0) AS sitepopularity, s.site_id, s.max_occupancy, s.accessible, s.max_rv_length, s.utilities, s.campground_id " 
				+ "FROM site s " 
	...
	public void createSite(Site site) {
		String sqlRowSetSite = "INSERT INTO site (site_id, campground_id, site_number, max_occupancy, accessible, max_rv_length, utilities) "
				+ "VALUES (" + site.getSiteId() + ", " + site.getCampgroundId() + ", " + site.getSiteNumber() + ", " + site.getMaxOccupancy() + ", " + site.isAccessible() + ", " + site.getMaxRvLength() + ", " + site.isUtilities() + ")";
		jdbcTemplate.update(sqlRowSetSite);
	}
